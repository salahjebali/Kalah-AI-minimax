...
public class Board {
	...
	private void checkFrontOpponent(int player, int lastIndex) {
		...
		if ((checkBorders(actualIndex, player) && stones == 0)) {
			...
			board[getMancalaIndex(player)] = board[getMancalaIndex(player)] + 1 + stolenStones;
		}
	...
	public boolean checkEndGame() {
		...
		if (checkEndMancala()) {
			result = true;
		} else if (checkEmptyDells(0) || checkEmptyDells(1)) {
			result = true;
			fillMancala();
		}
	...
	private void fillMancala() {
		int player = 0;
		for (int i = (size / 2) * player; i < getMancalaIndex(player); i++) {
			board[getMancalaIndex(player)] = board[getMancalaIndex(player)] + board[i];
			board[i] = 0;
		}
		player = (player + 1) % 2;
		for (int i = (size / 2) * player; i < getMancalaIndex(player); i++) {
