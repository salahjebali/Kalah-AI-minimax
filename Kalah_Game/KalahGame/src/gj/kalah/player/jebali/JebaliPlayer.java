...
public class JebaliPlayer implements Player {
	...
	public void tellMove(int move) { // move è la mossa avversaria
		game.moveDell(1, move); // la faccio eseguire sulla board
	}
	...
	private int minmax(Board board, int player, int depth, boolean playAgain, int[] alfabeta) {
		...
		int value = (player % 2 == 0) ? Integer.MIN_VALUE : Integer.MAX_VALUE;
		...
		for (int move : possibleMoves) {
			...
			value = (player % 2 == 0) ? Math.max(value, score) : Math.min(value, score);
			alfabeta = (player % 2 == 0) ? new int[] { Math.max(alfabeta[0], value), alfabeta[1] }
	...
	private int valuateBoard(Board board, int depth) {
		...
		if (board.checkEndGame()) { // se la partita è finita
			if (board.checkWinner(0)) { // se ho vinto io
				score = Integer.MAX_VALUE - depth; // assegno il punteggio massimo
			} else if (board.checkWinner(1)) { // altrimenti controllo se è stato l'avversario a vincere
				score = Integer.MIN_VALUE + depth; // in tal caso assegno il punteggio minimo
			}
		} else {
			score = board.getMancala(0) - board.getMancala(1);
		}
		return score;
	}
